...
{
    allowNone = Util.getProperty(allowNone, false);

    var result = allowNone;
    if(Util.defined(obj[name]))
    {
        var val = obj[name];
        min = Util.getProperty(min, -Number.MAX_VALUE);
        max = Util.getProperty(max, Number.MAX_VALUE);
        if(!isNaN(val))
        {
...
function validateArrayProperty(obj, name, allowNone)
{
    ...
    assertOrLog(result, name + ' must be assigned a valid window array');
}
...
function validateWithBaseFallback(func, obj, name)
{
    ...
    var preErrorCount = errorArray.length;
    func.apply(null, args.slice(1));
    if(errorArray.length > preErrorCount)
    {
...
function validateDialogSettings(dialogObject)
{
    ...
    if(dialogObject.hasOwnProperty('s'))
    {
        ...
        if(Util.defined(settings['sia']))
        {
            ...
            validateNumberProperty(sia, 'yo', true);
        }
        ...
        var optionSettings = Util.getProperty(
            settings['o'],
            (Util.defined(settings['base']) ? Util.getProperty(settings.base['o'], null) : null));
        if(optionSettings != null)
        {
            validateNumberProperty(optionSettings, 'x');
            validateNumberProperty(optionSettings, 'y');
            validateNumberProperty(optionSettings, 'w', true);
            validateNumberProperty(optionSettings, 'yo', true);
            validateNumberProperty(optionSettings, 'xo', true);
        }
        else if(Util.defined(dialogObject['sa']))
        {
            errorArray.push('Dialogs with options should have a s.o (settins-options) defined!');
        }
