...
({
	...
	init: function()
	{
		...
		if(!this.baseMenuData.hasOwnProperty('mi'))
		{
				// fine...
...
*/
	...
	update: function()
	{
		...
		if(ig.input.pressed ('primary'))
		{
			...
			{
                var nextMenu = Util.getProperty(ig.global.data.menus[menuItem.m], null);
                if(null != nextMenu)
                {
                    this.menuExit = true;
                    ModeStack.push('CustomMenu', this.baseMenuData, 'CustomMenu', nextMenu);
                }
                else
                {
			...
			{
				this.menuExit = true;
				ModeStack.push('CustomMenu', this.baseMenuData, menuItem.g, null);
			}
		}
