...
Dialog.prototype.changeToState = function(nextState)
{
    ...
    if(typeof nextState !== 'undefined' && nextState != null)
    {
        ...
        {
            // TODO: use state map on load to simplify this!
        ...
        y += heightData.height + 10; // TEMP HACK to buffer the options away from the text
        // TODO: options should be allowed to specify their own rect for display
        this.stateChoices = [];
        if(this.activeState.hasOwnProperty('o'))
        {
            ...
            {
                // Note: default option is a bit awkward as the number of options may differ based on state
                // TODO: consider making the default index into something better associated with an option
                    this.selectedIndex = 0;
            }
        }
        ...
        if(this.activeState != null)
        {
            ...
            if(this.activeState.hasOwnProperty('o') && this.activeState.o != null)
            {
                if(this.selectedIndex < this.stateChoices.length)
                {
                    ...
                    // if the option doesn't have a next state, try the active state
                    // TODO: this doesn't actually re-evaluate the dialog if the state has changed, critical to conversation style dialog
                    // may require more strict rules about ns:null indicating exit vs. ns:undefined to select the next state
...
Dialog.prototype.draw = function()
{
    ...
    for( var idx = 0, len = this.stateChoices.length; idx < len; idx++)
    {
            ...
            if (this.selectedIndex == idx)
            {
                    ...
                    {
                        var dialogIconSettings = this.getSetting('sia', null);
                        ...
                        else
                        {
                            ...
                            //TODO: can default everything in sia
                            this.selectedIcon.draw(stateChoice.x - this.getSetting('osx', 0), stateChoice.y,
                                    dialogIconSettings.sx,
                                    dialogIconSettings.sy,  
                                    dialogIconSettings.w,
                                    dialogIconSettings.h);
                        }
                    }
            }
            // draw the text
