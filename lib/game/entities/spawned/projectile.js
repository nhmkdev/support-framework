...
({
	...
	init: function( x, y, settings ) 
    {
        ...
        {
            this.kill();
            // TODO: may want to still try to hit the wall (non-critical...)
            ig.log('Projectile killed due to spawning in wall');
            return;
        }
    ...
    {
        var xL = this.pos.x;
        var xR = this.pos.x + this.size.x;
        var yT = this.pos.y;
        var yB = this.pos.y + (this.size.y - 1);

        //Particle.spawn(x, yT, { p:Particles.single});
        //Particle.spawn(x, yB, { p:Particles.single});
        return 0 !=
            ig.game.collisionMap.getTile(xL, yT) +
            ig.game.collisionMap.getTile(xL, yB) +
            ig.game.collisionMap.getTile(xR, yT) +
            ig.game.collisionMap.getTile(xR, yB);
    },
