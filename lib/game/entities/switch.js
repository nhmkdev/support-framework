...
({
	...
	init: function( x, y, settings )
	{
        ...
        if(!ig.global.wm)
        {
            for(stateName in this.interact.ao.a)
            {
                var anim = this.interact.ao.a[stateName];
                if(!Util.defined(anim['v']))
                {
    ...
    updateAnimationState: function()
    {
        ...
        for(stateName in this.interact.ao.a)
        {
            var animState = this.interact.ao.a[stateName];
            if(animState.v == flagVal)
            {
                Util.setAnimationState(this, animState, stateName);
                break;
            }
        }
    },
